...
fn apply_slope(N:usize,grid:&mut [f64],oi:usize,oj:usize){
    let s0=rnd::rangef(-10.,10.);
    let s1=rnd::rangef(-10.,10.);
    let mut k=0;
    for i in 0..N{
        for j in 0..N{
            grid[k]+=(i-oi) as i64 as f64*s0-(j-oj) as i64 as f64*s1;
            k+=1;
        }
    }
}
...
fn apply_sincos(N:usize,grid:&mut [f64],oi:usize,oj:usize){
    let s0=rnd::rangef(-0.4,0.4);
    let s1=rnd::rangef(-0.4,0.4);
    let amp=rnd::rangef(10.,100.);
    let mut k=0;
    for i in 0..N{
        for j in 0..N{
            grid[k]+=amp*(((i-oi) as i64 as f64*s0).sin()+((j-oj) as i64 as f64*s1).cos());
            k+=1;
        }
    }
}
...
fn apply_xor(N:usize,grid:&mut [f64],oi:usize,oj:usize){
    let mut k=0;
    for i in 0..N{
        for j in 0..N{
            grid[k]+=((i-oi)^(j-oj)) as i64 as f64;
            k+=1;
        }
    }
}
...
fn generate(N:usize,grid:&mut [f64]){
    for _ in 0..rnd::range(MINE,MAXE+1){
        ...
        let oj=rnd::next()%(N+1);
        match rnd::next()%11{
            0=>apply_slope(N,grid,oi,oj),
            1=>apply_sincos(N,grid,oi,oj),
            2=>apply_xor(N,grid,oi,oj),
            _=>apply_xy2(N,grid,oi,oj),
        }
    }
...
fn hide(N:usize,ans:&[f64],sampled:&mut [f64],flag:&mut [bool],bf:&mut Vec<(usize,usize,f64)>){
    bf.clear();
    ...
    for _ in 0..rnd::range(MINH,MAXH+1){
        ...
        loop{
            i=rnd::range(1,N-1);
            j=rnd::range(1,N-1);
            if !DIJ.into_iter().any(|(di,dj)|flag[(i+di)*N+j+dj]){
                break;
            }
        }
...
fn main(){
    for N in [24,32,40,48,56,64]{
        ...
        let mut flag=vec![false;N*N];
        let mut bf=vec![];
        let mut sampled=vec![0.;N*N];

        let data_dir=format!("data/data_{N}");
        ...
        for i in 0..COUNT{
            ...
            for i in 0..N*N{
                write!(file,"{} ",flag[i] as usize).unwrap();
            }
            file.flush().unwrap();
        }
